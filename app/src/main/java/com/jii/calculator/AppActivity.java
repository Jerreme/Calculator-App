    ...
    private void calculateResult(char operator) {
        ...
        switch (operator) {
            ...
            case '÷':
                ...
                break;
            default:
                Toast.makeText(AppActivity.this, "Error: Invalid Operator", Toast.LENGTH_SHORT).show();
        }
        ...
        if (result_val.length() > 15) {
            resultText.setTextSize(32);
        } else if (result_val.length() <= 18 && result_val.length() > 10) {
            resultText.setTextSize((prevQuantity * prevSize) / active_val.length() + (prevQuantity % 2));
        } else {
            resultText.setTextSize(54);
        }
